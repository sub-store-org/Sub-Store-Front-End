<template>
  <div @touchstart="onTouchStart" @touchmove="onTouchMove" @touchend="onTouchEnd" style="overflow: hidden;">
    ...
    <ArtifactPanel v-if="isEditPanelVisible" :name="editTargetName" @close="closeArtifactPanel" />
    ...
    <div v-if="artifacts.length > 0" class="subs-list-wrapper">
      ...
      <div class="sticky-title-wrapper sync-title">
        ...
        <div class="actions-wrapper">
          <nut-button class="upload-all-btn btn" type="info" plain :disabled="uploadAllIsDisabled" size="small"
            :loading="uploadAllIsLoading" @click="uploadAll">
            <font-awesome-icon icon="fa-solid fa-cloud-arrow-up" v-if="!uploadAllIsLoading" />
          </nut-button>
          <nut-button class="preview-btn btn" type="info" plain size="small" @click="preview" v-if="artifactStoreUrl">
    ...
    <!--没有数据-->
    <div v-if="!isLoading && fetchResult && artifacts.length === 0" class="no-data-wrapper">
      ...
      <nut-button @click="onclickAddArtifact" type="primary">{{ $t(`syncPage.emptySub.btn`) }}
      </nut-button>
    </div>
    ...
    <div v-if="!isLoading && !fetchResult" class="no-data-wrapper">
      ...
      <nut-button icon="refresh" type="primary" @click="refresh">{{ $t(`subPage.loadFailed.btn`) }}
      </nut-button>
