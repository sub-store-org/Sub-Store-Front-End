<template >
  ...
  >
    <div class="profile-block">
      <div class="info">
        <div class="avatar-wrapper">
          <nut-avatar :class="{ 'avatar-normal': !githubUser }" size="72" :icon="displayAvatar" />
          <div class="name">
            <p class="title">
        ...
        <div class="actions">
          <nut-button class="upload-btn" plain type="primary" :disabled="syncIsDisabled" size="small"
            :loading="uploadIsLoading" @click="sync('upload')">
            <font-awesome-icon icon="fa-solid fa-cloud-arrow-up" v-if="!uploadIsLoading" />
            {{ $t(`myPage.btn.upload`) }}</nut-button>
          <nut-button class="download-btn" type="primary" size="small" :disabled="syncIsDisabled"
            :loading="downloadIsLoading" @click="sync('download')">
            <font-awesome-icon v-if="!downloadIsLoading" icon="fa-solid fa-cloud-arrow-down" />
      ...
      <div class="config-card">
        ...
        <div class="config-input-wrapper">
          <nut-input class="input" v-model="userInput" :disabled="!isEditing"
            :placeholder="$t(`myPage.placeholder.githubUser`)" type="text" input-align="left" :left-icon="iconUser" />
          <nut-input class="input" v-model="tokenInput" :disabled="!isEditing"
            :placeholder="$t(`myPage.placeholder.gistToken`)" type="text" input-align="left" :left-icon="iconKey" />
        </div>
      </div>
    ...
    <div class="env-block">
      <img v-if="backendIcon()" :src="backendIcon()" alt="" class="auto-reverse" />
      <p>v{{ env.version }}</p>
      <p>{{ env.backend }}</p>
