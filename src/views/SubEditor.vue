...
const urlValidator = (val: string): Promise<boolean> => {
  ...
  const syncSubscriptionsFromRows = (rows: SubSelectRow[]) => {
    ...
    const selectedSet = new Set(subscriptions);
  ...
  const visibleSelectedSubscriptions = computed<string[]>({
    ...
    set: (visibleSelectedNames) => {
      ...
      const nextSelectedSet = new Set([
        ...selectedSubscriptions.value.filter((name) => !isRowVisibleByName(name)),
        ...visibleSelectedNames,
      ]);
      const nextSubscriptions = orderedSubsSelectList.value
        .map(([name]) => name)
        .filter((name) => nextSelectedSet.has(name));
  ...
  const subCheckboxClick = () => {
    ...
    if (subCheckboxIndeterminate.value) {
      ...
      for (let i = 0; i < group.length; i++) {
        const index = subscriptions.indexOf(group[i])
        if (index === -1) {
          subscriptions.push(group[i])
        }
      }
  ...
  watch([orderedSubsSelectList, tag], () => {
