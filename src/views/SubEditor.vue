    ...
    <div class="form-block-wrapper">
      ...
      <nut-form class="form" :model-value="form" ref="ruleForm">
        ...
        <nut-form-item required :label="$t(`editorPage.subConfig.basic.name.label`)" prop="name" :rules="[
          ...
          <input class="nut-input-text" @blur="customerBlurValidate('name')" v-model.trim="form.name"
        ...
        <nut-form-item :label="$t(`editorPage.subConfig.basic.icon.label`)" prop="icon">
          <input class="nut-input-text" v-model.trim="form.icon"
            :placeholder="$t(`editorPage.subConfig.basic.icon.placeholder`)" type="text" />
        </nut-form-item>
        ...
        <template v-if="editType === 'subs'">
          ...
          <nut-form-item required :label="$t(`editorPage.subConfig.basic.source.label`)" prop="source">
            <div class="radio-wrapper">
              <nut-radiogroup direction="horizontal" v-model="form.source">
                <nut-radio shape="button" label="remote">{{ $t(`editorPage.subConfig.basic.source.remote`) }}
                </nut-radio>
                <nut-radio shape="button" label="local">{{ $t(`editorPage.subConfig.basic.source.local`) }}
                </nut-radio>
              </nut-radiogroup>
            </div>
          </nut-form-item>
          <!-- url / content -->
          <nut-form-item required v-if="form.source === 'remote'" :label="$t(`editorPage.subConfig.basic.url.label`)"
            prop="url" :rules="[
              {
                required: true,
            ...
            <nut-textarea class="textarea-wrapper" @blur="customerBlurValidate('url')" v-model="form.url"
          ...
          <nut-form-item v-else-if="form.source === 'local'" :label="undefined" prop="content">
            <nut-textarea class="textarea-wrapper" v-model="form.content" text-align="left"
              :autosize="{ maxHeight: 410, minHeight: 50 }" :placeholder="$t(`editorPage.subConfig.basic.content.placeholder`)
                " type="text" />
          </nut-form-item>
          <!-- ua -->
          <nut-form-item :label="$t(`editorPage.subConfig.basic.ua.label`)" prop="ua" v-if="form.source === 'remote'">
            <input class="nut-input-text" v-model.trim="form.ua"
              :placeholder="$t(`editorPage.subConfig.basic.ua.placeholder`)" type="text" />
          </nut-form-item>
        </template>
        <template v-else-if="editType === 'collections'">
          <nut-form-item :label="$t(`editorPage.subConfig.basic.subscriptions.label`)" prop="subscriptions"
            class="include-subs-wrapper">
            <nut-checkboxgroup v-model="form.subscriptions" class="subs-checkbox-wrapper">
              <nut-checkbox v-for="item in subsSelectList" :key="item[0]" :label="item[0]" text-position="left"
                class="subs-checkbox">
                <div class="sub-img-wrapper">
                  <nut-avatar class="sub-item-customer-icon" v-if="item[2]" size="32" :url="item[2]"
                    bg-color=""></nut-avatar>
                  <span>{{ item[1] }}</span>
                </div>
              </nut-checkbox>
