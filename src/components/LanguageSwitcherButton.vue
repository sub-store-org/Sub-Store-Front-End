<template>
  ...
  >
    ...
    />
    <span v-if="props.variant !== 'icon'" class="language-switch-button__label">
...
const alternateLocale = computed(() =>
  currentLocale.value === "zh" ? "en" : "zh",
);
const alternateLanguageLabel = computed(() =>
  // t(`navBar.langSwitcher.${alternateLocale.value}`),
  t(`navBar.langSwitcher.language`)
);
